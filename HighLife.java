public class HighLife extends Thread {
    ...
    public HighLife(int start, int finish, int[][] primeiraMatriz, int[][] segundaMatriz, int geracao, int N) {
        this.start = start;
        this.finish = finish;
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        for (int i = 1; i <= generations; i++) {
            if (maxThreads == 0) { // roda a versão serial
                HighLife highLifeInstancia = new HighLife(0, size, primeiraMatriz, segundaMatriz, i, size);
                if (highLifeInstancia.geracao % 2 == 1) {
                    highLifeInstancia.setNewGeneration(highLifeInstancia.primeiraMatriz, highLifeInstancia.segundaMatriz);
                    imprime(primeiraMatriz);
                    System.out.println("Geração "+ i +": " + getVivos(primeiraMatriz, size));
                }
                else {
                    highLifeInstancia.setNewGeneration(highLifeInstancia.segundaMatriz, highLifeInstancia.primeiraMatriz);
                    imprime(segundaMatriz);
                    System.out.println("Geração "+ i +": " + getVivos(primeiraMatriz, size));
                }
            } else { // roda a versão com threads
                HighLife[] highLifeInstancia = new HighLife[maxThreads];
                for (int j = 0; j < maxThreads; j++) {
                    highLifeInstancia[j] = new HighLife((size / maxThreads) * j, (size / maxThreads) * (j + 1), primeiraMatriz, segundaMatriz, i, size);
                    highLifeInstancia[j].start();
                }
                for (int j = 0; j < maxThreads; j++) {
                    highLifeInstancia[j].join();
                }
            }
        }
    ...
    private void setNewGeneration(int[][] geracaoVelha, int[][] geracaoNova) {
        ...
        for (int i = this.start; i < this.finish; i++) {
            for (int j = 0; j < this.N; j++) {
                ...
                else if (geracaoVelha[i][j] == 0 && (vivos == 3 || vivos == 6)) geracaoNova[i][j] = 1;
